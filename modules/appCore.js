...
class AppCore {
	...
	loadParableText() {
		...
		parableContent.innerHTML = `
			...
			<p>Оставшись одна, девушка всё же не теряла надежды. Каждую ночь она выходила к воде и зажигала свечу. Говорят, и по сей день те, кто ищут истинную любовь, видят на озере свечу Светоносной девы, что надеется указать дорогу <span class="color-picker-trigger" style="cursor: pointer; font-weight: bold; position: relative; display: inline-block; border-bottom: none;">любимому<input type="color" class="hidden-color-picker" value="#ff0000" style="position: absolute; opacity: 0; width: 100%; height: 100%; left: 0; top: 0; cursor: pointer;"></span>.</p>
		`;
