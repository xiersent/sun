...
if (typeof WavesManager === 'undefined') {
    class WavesManager {
        ...
        updatePosition() {
            window.appState.virtualPosition = window.appState.currentDay * window.appState.config.squareSize;
            
            window.appState.data.waves.forEach(wave => {
                const wavePeriodPixels = window.appState.periods[wave.id] || (wave.period * window.appState.config.squareSize);
                let actualPosition = window.appState.virtualPosition % wavePeriodPixels;
                if (actualPosition < 0) {
                ...
                if (this.waveContainers[wave.id]) {
                    ...
                    let actualPositionNew = window.appState.virtualPosition % wavePeriodPixelsNew;
    ...
    window.WavesManager = WavesManager;
}

// Создаем экземпляр WavesManager
